...
- (void) setVideoIdentifier:(NSString *)videoIdentifier
{
	...
	{
        //なにこれ？
        NSLog(@"[XCDYoutube setVideoIdentifier:id] の if (![NSThread isMainThread]) なう");
...
- (void) startVideoInfoRequest
{
	...
	NSURL *videoInfoURL = [NSURL URLWithString:[NSString stringWithFormat:@"https://www.youtube.com/get_video_info?video_id=%@%@&ps=default&eurl=&gl=JP&hl=%@", self.videoIdentifier ?: @"", elField, ApplicationLanguageIdentifier()]];
...
{
    NSLog(@"XCDYoutubePlayer  finishWithError : %@", error);
...
{
    NSLog(@"YoutubePlayer : connectionDidFinishLoading");
	NSError *error = nil;
